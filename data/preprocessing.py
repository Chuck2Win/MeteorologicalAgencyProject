...
class preprocessing:
    def __init__(self):
        ...
        self.data['Title'] = self.data['title'].apply(lambda i : cleansing(i))
        self.data['Title'] = self.data['maintext'].apply(lambda i : cleansing(i))
        ...
        self.data['ids'] = self.data['Total'].apply(lambda i : tokenizer.encode(i,add_special_tokens=True,truncation=True,padding='max_length',max_length=512))
        # attention mask - mask될 부분은 0, 아닌 부분은 1
        self.data['mask']=(torch.tensor(self.data['ids'].tolist()).eq(1)==0).long().tolist()
    def return_data(self):
        self.data.loc[:,['ids','mask','len','damage','Total']].to_pickle('./data/preprocessed_data')
